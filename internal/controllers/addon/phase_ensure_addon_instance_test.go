	...
	t.Run("ensures AddonInstance", func(t *testing.T) {
		addonInstance := &addonsv1alpha1.AddonInstance{
			...
			Spec: addonsv1alpha1.AddonInstanceSpec{
				HeartbeatUpdatePeriod: common.DefaultAddonInstanceHeartbeatUpdatePeriod,
			},
		}
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				...
				assert.Equal(t, common.DefaultAddonInstanceHeartbeatUpdatePeriod, createdAddonInstance.Spec.HeartbeatUpdatePeriod)
			})
		}
	})
