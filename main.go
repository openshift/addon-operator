...
func setupWebhook() error {
	...
	if err != nil {
		return fmt.Errorf("unable to start webhook: %w", err)
	}
	...
	if err = wbHandler.InjectDecoder(admission.NewDecoder(mgr.GetScheme())); err != nil {
		return fmt.Errorf("unable to inject decoder: %w", err)
	}
	...
	if err := mgr.Start(ctrl.SetupSignalHandler()); err != nil {
		return fmt.Errorf("problem running webhook: %w", err)
	}
	return nil
}
...
func main() {
	...
	if err := setupWebhook(); err != nil {
		setupLog.Error(err, "setting up webhook")
		os.Exit(1)
	}
}
